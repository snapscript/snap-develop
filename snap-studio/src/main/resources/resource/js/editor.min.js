...
".json"))try{var m=JSON.parse(a);k=JSON.stringify(m,null,3);break a}catch(S){}k=a}else k="";l=C(d);a=k;l&&l.text&&l.lastModified>c?(console.log("LOAD FROM HISTORY diff\x3d["+(l.lastModified-c)+"]"),a=l.text):console.log("IGNORE HISTORY");c=y();c.resource&&c.source&&(l=G.md5(c.source),m=b.editorPanel.getValue(),m=D()?m:null,b.editorHistory[c.resource.resourcePath]={hash:l,lastModified:c.lastModified,history:c.history,position:c.position,buffer:m});n!=h&&g.setMode({path:n,v:Date.now()});b.editorPanel.setReadOnly(!1);
