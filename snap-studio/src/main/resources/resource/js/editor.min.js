...
!1)}function r(){if(null!=d.getEditorResource()){var c=d.getHistoryForResource(d.getEditorResource()),a=d.getEditorPanel().getValue(),c=c.getOriginalText();return a!=c}return!1}function t(c){if(K(c))return r();c=v.FileTree.createResourcePath(c);var a=d.getHistoryForResource(c);c=a.getOriginalText();a=a.getSavedText();return c!=a}function K(a){return null!=d.getEditorResource()&&d.getEditorResource().getResourcePath()==a?!0:!1}function L(){var a=d.getEditorPanel().getSession();d.getHistoryForResource(d.getEditorResource()).restoreScrollPosition(a,
d.getEditorPanel())}function F(a){z.Project.markEditorTab(a,t(a))}function M(){return{getCompletions:function(a,e,g,R,m){e=a.getValue();var c=a.session.getLine(g.row);a=d.getEditorResource().getProjectPath();c=c.substring(0,g.column);g=JSON.stringify({resource:a,line:g.row+1,complete:c,source:e,prefix:R});u.ajax({contentType:"application/json",data:g,dataType:"json",success:function(a){var c=a.expression,d=Math.max(0,c.lastIndexOf(".")+1);a=a.tokens;var e=[],g;for(g in a)if(a.hasOwnProperty(g)){var n=
a[g];b.Common.stringStartsWith(g,c)&&(g=g.substring(d));e.push({className:"autocomplete_"+n,token:g,value:g,score:300,meta:n})}m(null,e)},error:function(){console.log("Completion control failed")},processData:!1,type:"POST",url:"/complete/"+document.title})}}}function A(){var a=w.ace.edit("editor"),b=M();a.completers=[b];a.getSession().setMode("ace/mode/snapscript");a.getSession().setTabSize(3);a.setReadOnly(!1);a.setAutoScrollEditorIntoView(!0);a.getSession().setUseSoftTabs(!0);a.commands.removeCommand("replace");
a.commands.removeCommand("find");a.commands.removeCommand("expandToMatching");a.commands.removeCommand("expandtoline");a.setShowPrintMargin(!1);a.setOptions({enableBasicAutocompletion:!0});a.on("guttermousedown",function(c){var b=c.domEvent.target;if(-1!=b.className.indexOf("ace_gutter-cell")&&a.isFocused()&&!(c.clientX>25+b.getBoundingClientRect().left)){var e=c.getDocumentPosition().row,b=d.getEditorBreakpoints();if(null!=d.getEditorResource()){var g=d.getEditorPanel().getSession(),n=b[d.getEditorResource().getFilePath()],
f=g.getBreakpoints(),l=!1,k;for(k in f)if(k==e){l=!0;break}l?g.clearBreakpoint(e):g.setBreakpoint(e);k=parseInt(e);null==n?(n={},n[k+1]=!0,b[d.getEditorResource().getFilePath()]=n):n[k+1]=1==n[k+1]?!1:!0}p();c.stop()}});"undefined"!==typeof java&&a.setScrollSpeed(.05);return new H(a)}var d=null;a.createEditor=function(){d=A();d.activateEditor();N.EventBus.createTermination(e)};a.clearEditorHighlights=e;a.showEditorLine=function(a){d.getEditorPanel();d.getEditorPanel().resize(!0);1<a?(d.getEditorPanel().scrollToLine(a-
1,!0,!0,function(){}),d.getEditorPanel().gotoLine(a)):d.getEditorPanel().scrollToLine(0,!0,!0,function(){});d.getEditorPanel().focus()};a.createEditorHighlight=function(a,b){var c=w.ace.require("ace/range").Range,n=d.getEditorPanel().getSession(),m=d.getEditorMarkers();e();c=n.addMarker(new c(a-1,0,a-1,1),b,"fullLine");m[a]=c};a.createMultipleEditorHighlights=function(a,b){var c=w.ace.require("ace/range").Range,n=d.getEditorPanel().getSession(),m=d.getEditorMarkers();e();for(var h=0;h<a.length;h++){var f=
a[h],l=n.addMarker(new c(f-1,0,f-1,1),b,"fullLine");m[f]=l}};a.findAndReplaceTextInEditor=function(){var a=x().getResource();B.Command.searchAndReplaceFiles(a.getProjectPath())};a.findTextInEditor=function(){var a=x().getResource();B.Command.searchFiles(a.getProjectPath())};a.addEditorKeyBinding=function(a,b){d.getEditorPanel().commands.addCommand({name:a.editor,bindKey:{win:a.editor,mac:a.editor},exec:function(a){b&&b()}})};a.showEditorBreakpoints=p;a.resizeEditor=function(){var a=document.getElementById("editor").offsetWidth,
b=document.getElementById("editor").offsetHeight;console.log("Resize editor "+a+"x"+b);d.getEditorPanel().setAutoScrollEditorIntoView(!0);d.getEditorPanel().resize(!0)};a.currentEditorState=x;a.resolveEditorMode=y;a.clearSavedEditorBuffer=function(a){var c=v.FileTree.createResourcePath(a);d.getHistoryForResource(c).invalidateHistory();F(a)};a.currrentEditorBuffer=q;a.getEditorBufferForResource=l;a.updateEditor=function(c){var b=c.getResourcePath(),g=c.isHistorical()||c.isError(),f=c.getFileContent(),
m;m=c.getFileContent();var h=c.isHistorical()||c.isError();console.log("resource\x3d["+c.getResourcePath().getResourcePath()+"] modified\x3d["+c.getTimeStamp()+"] length\x3d["+c.getFileLength()+"] readonly\x3d["+h+"]");h?console.log("IGNORE HISTORY WHEN READ ONLY"):(h=l(c.getResourcePath().getResourcePath()),h.getSource()&&h.getLastModified()>c.getLastModified()?(console.log("LOAD FROM HISTORY diff\x3d["+(h.getLastModified()-c.getLastModified())+"]"),m=h.getSource()):console.log("IGNORE HISTORY: ",
h));c=d.getEditorPanel().getSession();var h=c.getMode(),q=y(b.getResourcePath()),r=x();if(!r.isReadOnly()){var t=r.getResource(),t=d.getHistoryForResource(t);r.isStateValid()?t.saveHistory(r):t.invalidateHistory()}q!=h&&c.setMode({path:q,v:Date.now()});g?(f=m,d.getEditorPanel().setReadOnly(!1),d.getEditorPanel().setValue(f,1)):(h=d.getHistoryForResource(b),d.getEditorPanel().setReadOnly(!1),d.getEditorPanel().setValue(m,1),h.updateHistory(m,f),h.restoreUndoManager(c,m));var f=d.getEditorPanel().getSession(),
k;for(k in f.$backMarkers)f.removeMarker(k);e();k=f.getBreakpoints();for(var v in k)f.clearBreakpoint(v);u("#currentFile").html("");d.getEditorPanel().setReadOnly(g);d.updateResourcePath(b,g);O.ProblemManager.highlightProblems();if(null!=b&&d.getEditorResource()&&(b=d.getEditorResource().getFilePath(),b=d.getEditorBreakpoints()[b],null!=b))for(var w in b)b.hasOwnProperty(w)&&1==b[w]&&(f=parseInt(w)-1,g=d.getEditorBreakpoints(),null!=d.getEditorResource()&&(v=d.getEditorPanel().getSession(),k=g[d.getEditorResource().getFilePath()],
f=parseInt(f),v.setBreakpoint(f),null==k&&(k={},g[d.getEditorResource().getFilePath()]=k),k[f+1]=!0),p());z.Project.createEditorTab();P.History.showFileHistory();Q.StatusPanel.showActiveFile(d.getEditorResource().getProjectPath());a.showEditorFileInTree();L();F(d.getEditorResource().getResourcePath())};a.showEditorFileInTree=function(){var a=x().getResource();v.FileTree.showTreeNode("explorerTree",a)};a.getCurrentLineForEditor=function(){return d.getEditorPanel().getSelectionRange().start.row};a.getSelectedText=
function(){return d.getEditorPanel().getSelectedText()};a.isEditorChanged=r;a.isEditorChangedForPath=t;a.scrollEditorToPosition=L;a.updateProjectTabOnChange=function(){d.getEditorPanel().on("input",function(){var a=d.getEditorResource(),a=d.getHistoryForResource(a),b=d.getEditorPanel().getValue();a.touchHistory(b);F(d.getEditorResource().getResourcePath())})};a.formatEditorSource=function(){var a=d.getEditorPanel().getValue(),b=d.getEditorResource().getFilePath();u.ajax({contentType:"text/plain",
data:a,success:function(a){d.getEditorPanel().setReadOnly(!1);d.getEditorPanel().setValue(a,1)},error:function(){console.log("Format failed")},processData:!1,type:"POST",url:"/format/"+document.title+b})};a.setEditorTheme=function(a){null!=a&&null!=d.getEditorPanel()&&d.getEditorPanel().setTheme(a)};a.updateEditorFont=function(a,b){var c=M();d.getEditorPanel().completers=[c];d.getEditorPanel().setOptions({enableBasicAutocompletion:!0,fontFamily:"'"+a+"',monospace",fontSize:b})}})(r=e.FileEditor||
(e.FileEditor={}))});
