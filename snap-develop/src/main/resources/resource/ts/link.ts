...
module LinkIndexer {
   ...
   const FUNCTION_REGEX = /(function|static|public|private|abstract|override|)\s+([a-z][a-zA-Z0-9]*)\s*\(/g;
   const VARIABLE_REGEX = /(var|const)\s+([a-z][a-zA-Z0-9]*)/g;
   const TYPE_REGEX = /(class|trait|enum)\s+([A-Z][a-zA-Z0-9]*)/g;
   const IMPORT_REGEX = /import\s+([a-z][a-zA-Z0-9\.]*)\.([A-Z][a-zA-Z]*)/g;
   
   const FUNCTION_TEMPLATES = ["%s("];
   const VARIABLE_TEMPLATES = ["%s.", "%s=", "%s =", "%s<", "%s <", "%s>", "%s >", "%s!", "%s !", "%s-", "%s -", "%s+", "%s +", "%s*", "%s *", "%s%", "%s %", "%s/", "%s /"];
   const TYPE_TEMPLATES = ["new %s(", "%s.", ":%s", ": %s", "extends %s", "with %s", "extends  %s", "with  %s", ".%s;", " as %s", "%s["];
   const IMPORT_TEMPLATES = ["new %s(", "%s.", ":%s", ": %s", "extends %s", "with %s", "extends  %s", "with  %s", ".%s;", " as %s", "%s["];
...
function createEditorLinks(editor, customMatchFunction, customOpenFunction) { // links for ACE editor
